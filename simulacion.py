...
class Animal(object):
    ...
    def puede_reproducir(self):
        if self.edad >= 2 and self.es_reproductore:
            return True
        else:
            return False
    ...
    def reproducirse(self, vecinos, lugares_libres):
        ...
        if vecinos:
            ...
            if type(self).__name__ == type(animal).__name__:
                if lugares_libres:
                    animal.tener_cria()
                    self.tener_cria()
                    pos = random.choice(lugares_libres)
                    self.es_reproductore = False
                    animal.es_reproductore = False
        return pos
...
class Leon(Animal):
    ...
    def prob_comer(self, animal):
        if self.edad in (0,1,2,3): p = 0.9
        if self.edad in (4,5,6,7): p = 0.5
        if self.edad in (8,9,10):  p = 0.9
        return random.random() > p
        #if self.edad * (1 - animal.edad)
...
class Mundo(object):
    ...
    def etapa_reproduccion(self):
        ...
        for p in self.tablero.posiciones_ocupadas():
            animal = self.tablero.posicion(p)
            animales_cercanos = self.tablero.posiciones_vecinas_con_ocupantes(p)
            lugares_libres = self.tablero.posiciones_vecinas_libre(p)
            desplazo = animal.reproducirse(animales_cercanos, lugares_libres)
            if desplazo:
                self.tablero.ubicar(desplazo, self.tablero.retirar(p))
